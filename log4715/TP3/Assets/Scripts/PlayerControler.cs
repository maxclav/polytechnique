...
public class PlayerControler : MonoBehaviour
{
    ...
    [SerializeField]
    AudioClip noBullet = null;

    [SerializeField]
    AudioClip reloadGun = null;
    ...
    void HorizontalMove(float axis)
    {
        if (_Grounded || _IsHooked)
        {
            ...
            _SpriteAnim.SetFloat("Speed", Mathf.Abs(horizontal));
        }
        else if (!_Grounded)
        {
    ...
    void CheckIsOnWall()
    {
        ...
        Vector3 back = transform.TransformDirection(Vector3.back * 0.4f);
        ...
        _SpriteAnim.SetBool("OnWall", _OnWall);
    }
    ...
    void CheckFire()
    {
        if (Input.GetButtonDown("Fire1") && _Gun.CanFire() && !_IsGunEmpty)
        {
            _PS.PlayShoot();
            _Gun.Fire();
        ...
        else if(Input.GetButtonDown("Fire1") && _IsGunEmpty)
        {
            _AS.PlayOneShot(noBullet, 0.35f);
        }
    }

    void CheckIfGrounded()
    {
        Debug.DrawLine(transform.position, transform.position + new Vector3(0, -1, 0) * 0.6f, Color.red);
        if (Physics.Raycast(transform.position, new Vector3(0, -1, 0), 0.6f, WhatIsGround))
        {
            _Grounded = true;
        }
        else
        ...
        _SpriteAnim.SetBool("Grounded", _Grounded);
    ...
    {        
        ...
        _SpriteAnim.SetBool("Hooked", _IsHooked);
